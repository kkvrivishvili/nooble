...
-- Eliminar políticas RLS previas que ya no son necesarias
DROP POLICY IF EXISTS ON app.document_collections;
DROP POLICY IF EXISTS ON app.documents;
DROP POLICY IF EXISTS ON app.document_chunks;
DROP POLICY IF EXISTS ON app.bot_collections;
DROP POLICY IF EXISTS ON app.bot_response_feedback;
DROP POLICY IF EXISTS ON app.vector_analytics;

-- Simplificar la seguridad de las tablas
...
-- Aplicar política única simplificada para todas las tablas del esquema app
CREATE POLICY "Require authentication for RAG tables"
ON ALL TABLES IN SCHEMA app
FOR ALL USING (auth.role() = 'authenticated');

-- Asegurar que las tablas públicas no tengan RLS
