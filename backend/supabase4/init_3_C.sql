...
DROP POLICY IF EXISTS ON app.user_roles;
DROP POLICY IF EXISTS ON app.user_role_history;
DROP POLICY IF EXISTS ON app.subscriptions;
DROP POLICY IF EXISTS ON app.subscription_history;

-- Habilitar RLS básico en tablas internas
ALTER TABLE app.user_roles ENABLE ROW LEVEL SECURITY;
ALTER TABLE app.user_role_history ENABLE ROW LEVEL SECURITY;
ALTER TABLE app.subscriptions ENABLE ROW LEVEL SECURITY;
ALTER TABLE app.subscription_history ENABLE ROW LEVEL SECURITY;

-- Aplicar política única simplificada
CREATE POLICY "User related tables access policy"
ON ALL TABLES IN SCHEMA app
FOR ALL USING (auth.role() = 'authenticated');

-- Asegurar que los perfiles sean públicos
