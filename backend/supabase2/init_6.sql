-- ARCHIVO: init.sql PARTE 6 - Funciones y Triggers
-- Propósito: Implementar funciones, triggers y procedimientos del sistema
...
-- Función para incrementar contador de clics
-- Corregido: Añadido SET search_path
CREATE OR REPLACE FUNCTION increment_link_clicks() 
RETURNS TRIGGER AS $$
BEGIN
    -- Solo incrementar si el link existe
    UPDATE links SET 
...
-- Función con permisos más granulares
-- Corregido: Añadido SET search_path
...
BEGIN
  ...
  -- Si tiene permissions específicos en JSONB, verificar
...
-- Función mejorada para registrar errores del sistema
-- Corregido: Añadido SET search_path
...
-- Función mejorada para verificar cuotas de usuario
-- Corregido: Añadido SET search_path
...
BEGIN
  ...
  v_quota := get_user_config_int(p_user_id, v_quota_key, 0);
  ...
  -- Actualizar métricas de uso mensual
...
-- Función mejorada para validar y convertir valores según tipo
-- Corregido: Añadido SET search_path
...
-- Función mejorada para establecer configuración de usuario
-- Corregido: Añadido SET search_path
...
BEGIN
  ...
  SELECT * INTO v_config_record FROM system_config WHERE key = p_config_key;
  IF NOT FOUND THEN
...
-- Función mejorada para aplicar configuración en masa por plan
-- Corregido: Añadido SET search_path
...
-- Función mejorada para reiniciar configuración de usuario a valores del plan
-- Corregido: Añadido SET search_path
CREATE OR REPLACE FUNCTION reset_user_config_to_plan(
  p_admin_id UUID,
  p_user_id UUID,
  p_config_key TEXT DEFAULT NULL -- NULL para resetear todas las configs
) RETURNS INTEGER AS $$
DECLARE
  v_plan VARCHAR(20);
...
BEGIN
  ...
  IF p_admin_id IS NULL OR p_user_id IS NULL THEN
    RAISE EXCEPTION 'Parámetros de administrador y usuario no pueden ser NULL';
  END IF;
  ...
  -- Si se especificó una clave específica
  IF p_config_key IS NOT NULL THEN
    -- Verificar que la configuración existe
    IF NOT EXISTS (SELECT 1 FROM system_config WHERE key = p_config_key) THEN
      RAISE EXCEPTION 'Configuración % no existe', p_config_key;
    END IF;
    
    -- Guardar valor antiguo para auditoría
    SELECT jsonb_build_object(p_config_key, value) INTO v_old_values
    FROM user_config
    WHERE user_id = p_user_id AND config_key = p_config_key;
    
    -- Eliminar la configuración específica de usuario
    DELETE FROM user_config
    WHERE user_id = p_user_id AND config_key = p_config_key
    RETURNING 1 INTO v_reset_count;
    
    -- Registrar la acción
    PERFORM log_system_error(
      p_admin_id,
      'config_reset',
      'Configuración ' || p_config_key || ' restablecida a valores del plan',
      'info',
      'admin_action',
      jsonb_build_object(
        'user_id', p_user_id,
        'config_key', p_config_key,
        'plan', v_plan,
        'old_value', v_old_values
      )
    );
  ELSE
    -- Guardar valores antiguos para auditoría
    SELECT jsonb_object_agg(config_key, value) INTO v_old_values
    FROM user_config
    WHERE user_id = p_user_id;
    
    -- Eliminar todas las configuraciones específicas de usuario
    DELETE FROM user_config
    WHERE user_id = p_user_id
    ...
    PERFORM log_system_error(
      p_admin_id,
      'config_reset_all',
      'Todas las configuraciones restablecidas a valores del plan',
      'info',
      'admin_action',
      jsonb_build_object(
        'user_id', p_user_id,
  ...
  INSERT INTO audit.config_changes (
    ...
    'reset:' || p_user_id::TEXT || COALESCE(':' || p_config_key, ''),
    v_old_values::TEXT,
    NULL,
    'RESET',
    p_admin_id::TEXT
...
-- Función mejorada para actualizar vector_dimensions
-- Corregido: Añadido SET search_path
...
-- Triggers para verificar cuotas con manejo de errores mejorado
-- Corregido: Añadido SET search_path
...
-- Función para regenerar embeddings para todos los documentos de una colección
-- Corregido: Añadido SET search_path
...
DO $$
BEGIN
  RAISE NOTICE 'Funciones y triggers creados correctamente con search_path establecido.';
END $$;