...
-- Verificación final para asegurar que la vista tiene SECURITY INVOKER
DO $$
DECLARE
  security_type TEXT;
BEGIN
  ...
  IF security_type = 'SECURITY DEFINER' THEN
    RAISE WARNING 'La vista public_bots todavía está configurada como SECURITY DEFINER. Recreando con SECURITY INVOKER...';
    
    -- Intentar recrearla una vez más
    DROP VIEW IF EXISTS public_bots CASCADE;
    
    EXECUTE 'CREATE VIEW public_bots
             WITH (security_invoker=true)
             AS SELECT
                b.id AS bot_id,
                b.name,
...
COMMENT ON TABLE profiles IS 'Información extendida de perfiles de usuario para minipages tipo Linktree';
...
COMMENT ON TABLE document_chunks IS 'Fragmentos de documentos vectorizados para búsqueda semántica con LlamaIndex';
