...
-- Crear un esquema dedicado para extensiones (recomendación de seguridad)
CREATE SCHEMA IF NOT EXISTS extensions;

-- Configurar search_path para incluir el esquema de extensiones
-- Corregido: Usar bloque DO para obtener nombre dinámico de BD
DO $$
BEGIN
  EXECUTE format('ALTER DATABASE %I SET search_path TO public, extensions, "$user"', current_database());
END $$;
...
-- Habilitar extensiones necesarias, moviéndolas al esquema extensions
...
-- Corregido: Añadido 'float' como tipo de dato permitido
...
-- Configuraciones API para SWR (nuevas)
  ...
  -- Nuevas configuraciones para LlamaIndex y Langchain
